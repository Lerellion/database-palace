...
export class TableService {
	...
	static async updateRecord(
		...
		try {
			const promise = fetch(`/api/data?table=${encodeURIComponent(tableName)}`, {
				method: 'PUT',
				headers: {
	...
	static async getTableData(
		...
		try {
			const promise = fetch(`/api/data?table=${encodeURIComponent(tableName)}`).then(
				response => this.handleResponse<{ columns: any[]; rows: any[] }>(response)
			)
