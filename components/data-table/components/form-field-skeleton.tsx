...
export function FormFieldSkeleton({ column, className }: FormFieldSkeletonProps) {
  ...
  const LabelSkeleton = () => (
    <div className="flex items-center gap-1">
      <div className="h-4 bg-muted rounded animate-pulse" style={{ width: `${column.name.length * 8}px` }} />
      {column.isNotNull && <div className="h-4 w-2 bg-destructive/20 rounded animate-pulse" />}
  ...
  return (
    <div className={cn("space-y-2", className)}>
      <LabelSkeleton />
      <div className={cn(
        "h-9 bg-muted rounded animate-pulse",
        getInputWidth()
      )} />
    </div>
  )
