...
function RecordFormContent({ tableName, columns, initialData, onSuccess, onCancel }: TProps) {
		...
		columns.forEach(column => {
			...
			if (value !== undefined && value !== null) {
				...
				if (column.maxLength && String(value).length > column.maxLength) {
					newErrors[column.name] =
						`${column.name} cannot exceed ${column.maxLength} characters`
					isValid = false
				}
			}
		})
								...
								>
									...
									<Switch
										...
										onCheckedChange={checked =>
											handleInputChange(column, checked)
										}
