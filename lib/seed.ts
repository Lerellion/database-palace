...
const sampleProducts = [
  { name: "Laptop", description: "High-performance laptop", price: 1299.99, category: "electronics", inStock: 10 },
  { name: "Smartphone", description: "Latest smartphone model", price: 899.99, category: "electronics", inStock: 15 },
  {
    name: "Headphones",
  ...
  { name: "Desk Chair", description: "Ergonomic office chair", price: 199.99, category: "furniture", inStock: 5 },
  { name: "Coffee Maker", description: "Automatic coffee maker", price: 89.99, category: "appliances", inStock: 8 },
  { name: "Backpack", description: "Durable travel backpack", price: 59.99, category: "accessories", inStock: 12 },
  { name: "Monitor", description: "27-inch 4K monitor", price: 349.99, category: "electronics", inStock: 7 },
]
...
export async function seedSqliteDb() {
  ...
  try {
    ...
    // Insert products
    const insertedProducts = await db.insert(sqliteSchema.products).values(sampleProducts).returning()
    ...
    const insertedOrderItems = await db.insert(sqliteSchema.orderItems).values(orderItems).returning()
...
export async function seedNeonDb() {
  ...
  try {
    ...
    try {
      ...
      console.error("Error: Tables don't exist in Neon database. Please run migrations first.")
      return false
    }
    ...
    // Insert products
    const insertedProducts = await db.insert(pgSchema.products).values(sampleProducts).returning()
    ...
    const insertedOrderItems = await db.insert(pgSchema.orderItems).values(orderItems).returning()
