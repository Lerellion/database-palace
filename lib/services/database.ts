...
function getDatabaseConfig() {
  ...
  if (!databaseUrl) {
    throw new Error('Database URL not provided. Set POSTGRES_URL or DATABASE_URL in your environment variables.');
  }
...
class DatabaseService {
  ...
  private getConnectionConfig(config: ConnectionConfig): DatabaseConfig {
    ...
    const isCloudProvider = !connectionString.includes('localhost') && !connectionString.includes('127.0.0.1');
