...
export async function POST(request: Request) {
    ...
    `)
		...
		const columnsSql = sql.join(
			insertColumns.map(c => sql.identifier(c)),
			sql`, `
		)
...
export async function PUT(request: Request) {
    ...
    `)
		...
		const setColumns = Object.entries(validatedData).map(
			([key, value]) => sql`${sql.identifier(key)} = ${sql.param(value)}`
		)

		const whereColumns = Object.entries(validatedWhere).map(
			([key, value]) => sql`${sql.identifier(key)} = ${sql.param(value)}`
		)
