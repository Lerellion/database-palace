...
import { Database, PencilIcon, PlusIcon, SearchIcon, TableIcon, TrashIcon } from 'lucide-react'
...
export default function TablesPage() {
	...
	if (!isConnected) {
		return (
			<div className="grid place-items-center w-full h-[calc(100vh-4rem)]">
			...
			<div className="grid gap-4">
				{isLoading ? (
					...
					<div className="text-red-500">
						{error}
						<Button onClick={fetchTables} variant="outline" size="sm" className="ml-2">
							Retry
						</Button>
					</div>
					...
					filteredTables.map(table => (
						<div key={table.name} className="p-4 border rounded-lg hover:bg-muted/50">
								...
								<div className="mt-4 pl-8">
									...
									<div className="grid gap-1">
										{table.columns.map((column: TableColumn) => (
											...
											>
												<span className={column.isKey ? 'font-medium' : ''}>
													{column.name}
												</span>
